...
class Decoder(nn.Module):
    ...
    def caption(self, encoded_img_features, word_map, beam_size=5):
        ...
        enc_img_size = encoded_img_features.size(1)
        ...
        prev_words = captions
        top_scores = torch.zeroes(beam_size, 1).to(device)
        
        
        # alpha weights 
        soft_weights = torch.ones(beam_size, 1, enc_img_size, enc_img_size)
        ...
        while True: 
            ...
            soft_weights = torch.cat((soft_weights[prev_word_idxs], soft_weights[prev_word_idxs].unsqueeze(1)), dim=1)
